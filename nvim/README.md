...
| `<leader><tab>]` / `[`  ·  `gt` / `gT` | Next / prev Neovim tab (sequential) |
| `{count}gt` | **Jump straight to tab N** by number — `2gt`, `3gt` (fastest with a handful of tabs) |
| `:BufferLineTabRename <name>` | Rename the current tab shown in the top bar |
| `<C-w>` then a key | The window namespace — split, move, resize (see [§1](#1-the-mental-model)) |

> **Neovim tabs vs. Zellij tabs.** `<leader><tab>*` (and native `gt`/`gT`, `{count}gt`) are Neovim
> *tabpages* — viewport arrangements inside the editor, shown in the top bar (bufferline `mode = "tabs"`,
> [config](./lua/plugins/bufferline.lua)). Your project tabs — editor │ agent — are **Zellij's**
> (`Ctrl+t`); see the [multiplexer doc](../zellij/README.md). Different layers, different keys — no collision.
...
| [`lua/plugins/bufferline.lua`](./lua/plugins/bufferline.lua) | `bufferline.nvim` → `mode = "tabs"` | Top bar shows **tabpages**, not buffers — one entry per tab, styled in the stack palette. Buffers stay hidden (never buffer mode). LazyVim's `always_show_bufferline = false` keeps the bar out until ≥2 tabs, so a new tab (`:tabnew`, `:DiffviewOpen`) appears on its own. Switch: `gt`/`gT` or `<leader><tab>]`/`[`; rename a tab with `:BufferLineTabRename`. Buffer cycling stays on native `<S-h>`/`<S-l>`, `[b`/`]b`. |
