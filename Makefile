...
.PHONY: help build try zellij check check-local bootstrap install update macos jetbrains cursor git-setup clean
...
cursor: ## Link Cursor settings/keybindings + vscode-neovim init.lua (opt-in; --dry-run)
	./cursor/install.sh
